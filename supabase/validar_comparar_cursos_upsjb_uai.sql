-- ============================================================
-- MI NOTA FINAL - VALIDACIÓN CONSOLIDADA DE CURSOS UPSJB + UAI
-- Objetivo: comparar cursos oficiales de malla contra cursos en Supabase.
-- Modo: SOLO LECTURA sobre datos permanentes. No inserta, no actualiza, no elimina.
-- Crea únicamente tablas temporales y funciones de normalización.
-- ============================================================
...
-- Config y facultades UPSJB para comparación.
...
-- Normalizador adicional usado en scripts UAI.
...
-- ============================================================
-- CONSOLIDADO OFICIAL PARA COMPARACIÓN
-- No modifica datos. Solo usa tablas temporales y consultas SELECT.
-- ============================================================

drop table if exists tmp_official_courses_all_raw;
create temp table tmp_official_courses_all_raw as
select
  'UPSJB'::text as university_code,
  cf.faculty_name,
  m.career_name,
  m.cycle_order as cycle_number,
  m.course_name
from tmp_malla_cursos_upsjb m
join tmp_career_faculty_upsjb cf
  on public.mn_normalize(cf.career_name) = public.mn_normalize(m.career_name)
union all
select
  'UAI'::text as university_code,
  uai.faculty_name,
  uai.career_name,
  uai.cycle_number,
  uai.course_name
from tmp_uai_official_courses uai;

drop table if exists tmp_official_courses_all;
create temp table tmp_official_courses_all as
select distinct on (
  public.mn_normalize(university_code),
  public.mn_normalize(faculty_name),
  public.mn_normalize(career_name),
  cycle_number,
  public.mn_normalize(course_name)
)
  university_code,
  faculty_name,
  career_name,
  cycle_number,
  course_name,
  public.mn_normalize(university_code) as norm_university,
  public.mn_normalize(faculty_name) as norm_faculty,
  public.mn_normalize(career_name) as norm_career,
  public.mn_normalize(course_name) as norm_course
from tmp_official_courses_all_raw
order by
  public.mn_normalize(university_code),
  public.mn_normalize(faculty_name),
  public.mn_normalize(career_name),
...
-- Cursos actuales de la BD dentro de UPSJB y UAI.
-- Se toma la relación real: courses -> careers -> faculties -> universities.
drop table if exists tmp_current_courses_all;
create temp table tmp_current_courses_all as
select
  c.id as course_id,
  ...
  cy.order_number as cycle_number,
  cy.name as cycle_name,
  c.name as course_name,
  coalesce(c.status, 'sin_estado') as status,
  c.created_at,
  c.updated_at,
  public.mn_normalize(u.code) as norm_university,
  public.mn_normalize(f.name) as norm_faculty,
  public.mn_normalize(ca.name) as norm_career,
  public.mn_normalize(c.name) as norm_course,
  c.university_id as course_university_id,
  c.faculty_id as course_faculty_id,
  c.career_id,
  c.cycle_id,
  f.university_id as faculty_university_id,
  ca.faculty_id as career_faculty_id
...
where public.mn_normalize(u.code) in (public.mn_normalize('UPSJB'), public.mn_normalize('UAI'));

-- ============================================================
-- 1) RESUMEN GENERAL: oficiales vs BD
-- ============================================================
select
  o.university_code as universidad,
  o.faculty_name as facultad,
  o.career_name as carrera,
  o.cycle_number as ciclo,
  count(*) as cursos_oficiales_malla,
  count(*) filter (
    where exists (
      select 1 from tmp_current_courses_all c
      where c.norm_university = o.norm_university
        and c.norm_faculty = o.norm_faculty
        and c.norm_career = o.norm_career
        and c.cycle_number = o.cycle_number
        and c.norm_course = o.norm_course
    )
  ) as existen_en_bd_cualquier_estado,
  count(*) filter (
    where exists (
      select 1 from tmp_current_courses_all c
      where c.norm_university = o.norm_university
  ...
  ) as faltan_totalmente
from tmp_official_courses_all o
group by o.university_code, o.faculty_name, o.career_name, o.cycle_number
order by o.university_code, o.faculty_name, o.career_name, o.cycle_number;

-- ============================================================
-- 2) CURSOS OFICIALES QUE FALTAN TOTALMENTE EN BD
-- Si este resultado sale vacío, no falta ningún curso oficial.
-- ============================================================
select
  o.university_code as universidad,
  o.faculty_name as facultad,
  o.career_name as carrera,
  o.cycle_number as ciclo,
  o.course_name as curso_oficial_faltante
from tmp_official_courses_all o
where not exists (
  select 1 from tmp_current_courses_all c
  where c.norm_university = o.norm_university
...
order by o.university_code, o.faculty_name, o.career_name, o.cycle_number, o.course_name;

-- ============================================================
-- 3) CURSOS OFICIALES QUE EXISTEN, PERO ESTÁN INACTIVOS
-- Estos no faltan, pero no aparecerán en la app hasta reactivarse.
-- ============================================================
select
  o.university_code as universidad,
  o.faculty_name as facultad,
  o.career_name as carrera,
  o.cycle_number as ciclo,
  o.course_name as curso_oficial,
  c.course_id,
  c.course_name as curso_en_bd,
  c.status
from tmp_official_courses_all o
join tmp_current_courses_all c
  on c.norm_university = o.norm_university
 and c.norm_faculty = o.norm_faculty
 and c.norm_career = o.norm_career
 and c.cycle_number = o.cycle_number
 and c.norm_course = o.norm_course
where c.status <> 'active'
  and not exists (
    select 1 from tmp_current_courses_all c2
    where c2.norm_university = o.norm_university
      and c2.norm_faculty = o.norm_faculty
      and c2.norm_career = o.norm_career
      and c2.cycle_number = o.cycle_number
      and c2.norm_course = o.norm_course
      and c2.status = 'active'
  )
order by universidad, facultad, carrera, ciclo, curso_oficial;

-- ============================================================
-- 4) CURSOS ACTIVOS EN BD QUE NO ESTÁN EN LA MALLA OFICIAL
-- Estos serían candidatos a baja lógica, salvo que sean electivos/otros válidos.
-- ============================================================
select
  c.university_code as universidad,
  c.faculty_name as facultad,
  c.career_name as carrera,
  c.cycle_number as ciclo,
  c.course_id,
  c.course_name as curso_activo_no_oficial,
  c.status
from tmp_current_courses_all c
where c.status = 'active'
  and exists (
    select 1 from tmp_official_courses_all o
    where o.norm_university = c.norm_university
      and o.norm_faculty = c.norm_faculty
      and o.norm_career = c.norm_career
  )
  and not exists (
    select 1 from tmp_official_courses_all o
    where o.norm_university = c.norm_university
      and o.norm_faculty = c.norm_faculty
      and o.norm_career = c.norm_career
      and o.cycle_number = c.cycle_number
      and o.norm_course = c.norm_course
  )
order by universidad, facultad, carrera, ciclo, curso_activo_no_oficial;

-- ============================================================
-- 5) CURSOS QUE COINCIDEN POR NORMALIZACIÓN, PERO TIENEN NOMBRE DIFERENTE
-- Ejemplo: sin tilde, mayúsculas raras, espacios, etc. Estos se pueden RENOMBRAR.
-- ============================================================
select
  o.university_code as universidad,
  o.faculty_name as facultad,
  ...
  o.cycle_number as ciclo,
  c.course_id,
  c.course_name as nombre_actual_bd,
  o.course_name as nombre_oficial_malla,
  c.status
from tmp_official_courses_all o
join tmp_current_courses_all c
  on c.norm_university = o.norm_university
 ...
 and c.cycle_number = o.cycle_number
 and c.norm_course = o.norm_course
where c.course_name <> o.course_name
order by universidad, facultad, carrera, ciclo, nombre_oficial_malla;

-- ============================================================
-- 6) DUPLICADOS REALES EN BD POR CONTEXTO + NOMBRE NORMALIZADO
-- Si aparece algo aquí, hay más de un registro representando el mismo curso.
-- ============================================================
select
  university_code as universidad,
  faculty_name as facultad,
  career_name as carrera,
  cycle_number as ciclo,
  norm_course as curso_normalizado,
  count(*) as total_registros,
  string_agg(course_id::text, ', ' order by course_id::text) as ids,
  string_agg(course_name || ' [' || status || ']', ' | ' order by course_name) as cursos
from tmp_current_courses_all
group by university_code, faculty_name, career_name, cycle_number, norm_course
having count(*) > 1
order by total_registros desc, universidad, carrera, ciclo, curso_normalizado;

-- ============================================================
-- 7) INCONSISTENCIAS DE CONTEXTO EN courses
-- Detecta si courses.university_id/faculty_id no coincide con career->faculty->university.
-- ============================================================
select
  c.id as course_id,
  c.name as curso,
  u.code as universidad_por_carrera,
  f.name as facultad_por_carrera,
  ca.name as carrera,
  cy.order_number as ciclo,
  c.university_id as course_university_id,
  f.university_id as expected_university_id,
  c.faculty_id as course_faculty_id,
  ca.faculty_id as expected_faculty_id,
  c.status
from public.courses c
join public.careers ca on ca.id = c.career_id
join public.faculties f on f.id = ca.faculty_id
...
where public.mn_normalize(u.code) in (public.mn_normalize('UPSJB'), public.mn_normalize('UAI'))
  and (
    c.university_id is distinct from f.university_id
    or c.faculty_id is distinct from ca.faculty_id
  )
order by universidad_por_carrera, facultad_por_carrera, carrera, ciclo, curso;

-- ============================================================
-- 8) CONTEO FINAL POR UNIVERSIDAD
-- ============================================================
select
  'OFICIAL_MALLA' as origen,
  university_code as universidad,
  count(*) as total_cursos
from tmp_official_courses_all
group by university_code
union all
select
  'BD_ACTIVOS' as origen,
  university_code as universidad,
  count(*) as total_cursos
from tmp_current_courses_all
where status = 'active'
...
union all
select
  'BD_TODOS_LOS_ESTADOS' as origen,
  university_code as universidad,
  count(*) as total_cursos
from tmp_current_courses_all
group by university_code
order by universidad, origen;
